...
import { Play, Pause, Volume2, VolumeX, Settings, ArrowLeft, Maximize, Minimize } from 'lucide-react';
      ...
      {showControls && (
        <div className="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/80 to-transparent z-[10001]">
          <div className="flex items-center gap-4">
            <button
              onClick={handleBack}
              className="text-white hover:text-netflix-red transition-colors duration-200 cursor-pointer p-2 rounded-full hover:bg-black/20 z-[10002]"
              aria-label="Go back"
            >
              <ArrowLeft size={24} />
            </button>
      ...
      {showControls && (
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent z-[10000]">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            ...
            >
              <div 
                className="h-full bg-netflix-red relative group-hover:h-2 transition-all"
                ...
                {availableQualities.length > 1 && (
                  <div className="relative">
                    <button
                      ...
                      className="text-white hover:text-netflix-red transition flex items-center gap-1"
                      ...
                      <div className="absolute bottom-full right-0 mb-2 bg-black/90 rounded-md overflow-hidden">
                        {availableQualities.map((quality) => (
                          <button
                            ...
                            className={`block w-full px-4 py-2 text-sm text-left hover:bg-netflix-red transition ${
                              currentQuality === quality.value ? 'bg-netflix-red' : ''
                            }`}
                          >
                            {quality.label}
                          </button>
